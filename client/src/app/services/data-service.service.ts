...
export class DataServiceService {
  ...
  getHouse(id:string):Observable<any>{
    return this.http.get(`http://localhost:3001/houses/${id}`)

  }

  setFavorite(houseId: string, userId: string) {
    this.http
      .put<any>(`http://localhost:3001/users/addfavoritehouse`, {
        houseId: houseId,
        userId: userId,
      })
      .subscribe({
  ...
  deleteFavorite(houseId: string, userId: string) {
    this.http
      .put<any>(`http://localhost:3001/users/deletefavoritehouse`, {
        houseId: houseId,
        userId: userId,
      })
      .subscribe({
  ...
  createHouse(house: NewHouse, email: string) {
    this.http
      .post(`http://localhost:3001/houses/createhouse?userMail=${email}`, house)
      .subscribe({
        error: (error) => {
          console.log(error);
        },
