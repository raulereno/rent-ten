...
router.post("/", async (req, res) => {
  ...
  try {
    if (!opinion || !rating || !userId || !houseId || !userEmail)
      res
        .status(400)
        .json({
          msg: "Missing some field: opinion, rating, userId, houseId, and userEmail are required.",
        });
  ...
  try {
    ...
    if (owner == userId) {
      ...
      return res
        .status(400)
        .json({
          msg: `The ID ${userId} is not the owner of the review with ID ${reviewId}`,
        });
    }
  } catch (error) {
    console.log(error);
